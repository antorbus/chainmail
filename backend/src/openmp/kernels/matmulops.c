...
FORWARD_FUNC_DEF(m_op_bmm_forward) {
    ...
    lemur_float* A = (lemur_float*) k0->array;
    lemur_float* B = (lemur_float*) k1->array;
    lemur_float* C = (lemur_float*) kr->array;

    for (size_t batch = 0; batch < bs; batch++) {
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                    (int)i, (int)j, (int)k, 1.0f, 
                    &A[batch * i * k], (int)k, 
                    &B[batch * k * j], (int)j, 
                    0.0f, &C[batch * i * j], (int)j);
    }
}
...
//                             lemur_float temp = A[_b][ii][kk];
//                             #pragma omp simd
//                             for (size_t jj = _j; jj < MIN(_j + TILE_SIZE, j); jj++) {
//                                  C[_b][ii][jj] +=  temp * B[_b][kk][jj];
//                             }
//                         }
//                     }
